...
cds.on ('bootstrap', app => {
  ...
  dataSourceURIs (env.folders.app).forEach(({appPath, dataSourceUri}) => {
    ...
    app.use(uiRoutes, ({originalUrl}, res, next)=> {
      ...
      // determine v2 proxy prefix
      if (!v2Prefix) v2Prefix = env.protocols?.['odata-v2']?.path || env.odata.v2proxy?.path || '/odata/v2'
      // v4 prefix can be configured by app dev
      const v4Prefix = env.protocols?.['odata-v4']?.path 
      ...
      const srv = serviceForUri[dataSourceUri] || (serviceForUri[dataSourceUri] =
        cds.service.providers.find (srv => {
          ...
          if (normalizedUri.lastIndexOf(srv.path) >=0) return true
          // is it a request to v2 proxy targeting a v4 service with protocol prefix?
          if (normalizedUri.lastIndexOf(srv.path.replace(v4Prefix,'')) >=0) return true
        })
      )
      if (srv) {

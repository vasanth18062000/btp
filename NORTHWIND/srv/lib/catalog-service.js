...
// const { Service } = require('@sap/cds/apis/services');
// const service = cds.service;
...
// 				const service =  cds.connect.to('Northwind');
...
module.exports = cds.service.impl(async function() {
	...
	this.on('READ',Persons,request => {
        LOG.info('Persons entity fetched');
		 return service.tx(request).run(request.query);
		// return service.tx(request).run(SELECT.from(Persons,1));
		// return service.tx(request).run(SELECT.from(Persons).where({ID:{'=':1}}));
	}),

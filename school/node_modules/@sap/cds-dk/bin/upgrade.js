...
async function upgrade([tenant], options = {}) {
    const { getMessage } = require('../lib/client/helper/logging');
    if (!tenant) {
        throw getMessage('Tenant not given.', { command: 'upgrade' });
    }
    ...
    if (options.local) {
        ...
        if (options.user) {
            throw getMessage('Username cannot be specified when running @sap/cds-mtxs locally.', { command: 'upgrade' });
        }
        if (options.at) {
            throw getMessage('URL cannot be specified when running @sap/cds-mtxs locally.', { command: 'upgrade' });
        }
    }

...
const books = Vue.createApp ({
    ...
    data() {
      return {
        list: [],
        book: undefined,
        order: { quantity:1, succeeded:'', failed:'',price:1,succeeded:'', failed:'' },
        user: undefined
      }
    },
    ...
    methods: {
        ...
        async submitMe(){
            ...
            const price=parseInt (book.price) || 1
             const quantity = parseInt (order.quantity) || 1 // REVISIT: Okra should be less strict
             const user=books.user
            try {
                const res = await POST(`/submitMe`, { quantity, book: book.ID,price,user })
                book.stock = res.data.stock
                books.order = { quantity, succeeded: `Successfully ordered ${quantity} item(s) of price ${price} for user${user}.` }
            } catch (e) {
                books.order = { quantity, failed: e.response.data.error ? e.response.data.error.message : e.response.data }
            }
        },

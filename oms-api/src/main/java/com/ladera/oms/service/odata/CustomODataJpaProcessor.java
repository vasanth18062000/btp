...
public class CustomODataJpaProcessor extends ODataJPADefaultProcessor {
	...
	public ODataResponse readEntitySet(final GetEntitySetUriInfo uriParserResultView, final String contentType)
			throws ODataJPAModelException, ODataJPARuntimeException, EdmException {
	...
	public ODataResponse readEntity(final GetEntityUriInfo uriParserResultView, final String contentType)
		...
		if (uriParserResultView.getKeyPredicates().size() > 1) {
			logger.info("READ: Entity {} called with key {} and key {}",
			...
			logger.info("READ: Entity {} called with key {}", uriParserResultView.getTargetEntitySet().getName(),
					uriParserResultView.getKeyPredicates().get(0).getLiteral());
		}
		try {
	...
	public ODataResponse updateEntity(final PutMergePatchUriInfo uriParserResultView, final InputStream content,
			...
			throws ODataException, ODataJPAModelException, ODataJPARuntimeException, ODataNotFoundException {
		logger.info("PUT: Entity {} called with key {}", uriParserResultView.getTargetEntitySet().getName(),
				uriParserResultView.getKeyPredicates().get(0).getLiteral());
		ODataResponse response = null;
		try {
	...
	public ODataResponse deleteEntity(DeleteUriInfo uriParserResultView, String contentType) throws ODataException {
		logger.info("DELETE: Entity {} called with key {}", uriParserResultView.getTargetEntitySet().getName(),
				uriParserResultView.getKeyPredicates().get(0).getLiteral());
